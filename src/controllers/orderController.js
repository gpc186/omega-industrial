...
async function createOrder(req, res) {
    try {
        ...
        const compra_id = await Order.create(userId, order_number, total_preco, items, req.body.notes || null);
        
        return res.status(201).json({message: "Pedido criado com sucesso!", compra_id, order_number, total_preco});
    } catch (error) {
        console.error(error);
...
async function getMyOrders(req, res) {
    try {
        ...
        if(!pedidos) return res.status(404).json({message: "Nenhum pedido ainda foi feito"});
